--CREATE MATERIALIZED VIEW yj_exclusion_flag AS WITH age_los AS (
         ...
         SELECT er.subject_id,
            ...
            er.intime,
            er.outtime,
            er.icu_length_of_stay,
            er.age,
            er.icustay_id_order,
            er.exclusion_los,
            er.exclusion_age,
            er.exclusion_first_stay,
            er.exclusion_hr,
            er.exclusion_rr,
                CASE
                    WHEN ((uf.urineoutput < (400)::double precision) OR (uf.urineoutput IS NULL)) THEN 1
                    ELSE 0
                END AS exclusion_uf
           FROM (ex_rr er
             LEFT JOIN mimiciii.uofirstday uf ON (((uf.subject_id = er.subject_id) AND (uf.icustay_id = er.icustay_id) AND (uf.hadm_id = er.hadm_id))))
        ), cre_value AS (
