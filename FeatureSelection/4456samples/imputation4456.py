...
#加载数据
data=pd.read_csv('./data/addsex4456.csv')
...
data.drop([ 'sirs','saps','diuretic'],inplace=True,axis=1)#去掉不合理的特征值si，利尿剂、以及两个不相关的评分
...
nullpercent=data.count()/4456#统计每个特征值的缺失比例
...
data.to_csv('./data/outlier4456.csv')#最终用于机器学习模型中的完整数据集
